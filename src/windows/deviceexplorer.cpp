...
std::vector<slk::DeviceDescriptor> DeviceExplorer::devices(slk::DeviceType type, slk::DeviceState state) const noexcept
{
    ...
    std::vector<slk::DeviceDescriptor> result;
...
slk::DeviceInfo DeviceExplorer::resolveDevice(const slk::DeviceDescriptor& desc) const noexcept
{
    if (!impl().enumerator) {
        return {};
    }
...
slk::DeviceInfo DeviceExplorer::resolveDefaultDevice(slk::DeviceType type, slk::Purpose purpose) const noexcept
{
    if (!impl().enumerator) {
        return {};
    }
